 ...
 * PERBAIKAN: Redirect menggunakan path relatif (bukan BASE_URL)
 */
...
class Auth {
     ...
     * PERBAIKAN: Redirect menggunakan path relatif
     */
    public static function checkAuth() {
        ...
        if (!isset($_SESSION['user_id']) || empty($_SESSION['user_id'])) {
            ...
            // Redirect ke login dengan path relatif (BUKAN BASE_URL)
            header('Location: /responsive-message-app/login.php?error=session_expired');
            exit;
        }
        ...
        if (!isset($_SESSION['user_type']) || empty($_SESSION['user_type'])) {
            ...
            header('Location: /responsive-message-app/login.php?error=invalid_session');
            exit;
        }
        ...
        if (isset($_SESSION['login_time']) && (time() - $_SESSION['login_time'] > SESSION_LIFETIME)) {
            ...
            self::logout();
            header('Location: /responsive-message-app/login.php?expired=1');
            exit;
        }
    ...
    public static function checkAdmin() {
        ...
        if ($user_type !== 'Admin' && $privilege_level !== 'Full_Access') {
            ...
            // Redirect ke dashboard sesuai role dengan path relatif
            if (in_array($user_type, ['Siswa', 'Guru', 'Orang_Tua'])) {
                header('Location: /responsive-message-app/modules/dashboard.php?error=access_denied');
            } elseif ($user_type === 'Wakil_Kepala' || $user_type === 'Kepala_Sekolah') {
                header('Location: /responsive-message-app/modules/wakepsek/dashboard.php?error=access_denied');
            } elseif (strpos($user_type, 'Guru') === 0) {
                header('Location: /responsive-message-app/modules/guru/followup.php?error=access_denied');
            } else {
                header('Location: /responsive-message-app/index.php?error=access_denied');
            }
            exit;
        }
    ...
    public function requestPasswordReset($email) {
        try {
            ...
            // Send reset email
            $resetLink = "/responsive-message-app/reset_password.php?token=" . urlencode($token) . "&id=" . $user['id'];
